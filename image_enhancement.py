...
def plot_histogram(image, title):
    """Plot histogram for each color channel in the image."""
...
# Assuming enhance_image is defined and performs the enhancement
if __name__ == "__main__":
    filename = "../images/lowLightImage.jpg"
    image = imageio.imread(filename)  # Load the image
    ...
    # 1. Apply Gray World Color Correction
    image_gray_world = correct_colors(image, verbose=False)
    plt.figure(figsize=(6, 6))
    plt.imshow(image_gray_world, vmin=0, vmax=255)
    plt.title('Gray World Color Correction')
    plt.axis('off')
    plt.tight_layout()
    plt.show()

    # 2. Apply Brightness Adjustment
    image_brightness = adjust_brightness(image, degree=0.1, verbose=False)

    # 3. Apply Local Contrast Enhancement
    image_contrast = apply_local_contrast_enhancement(
        image=rgb2gray(image.copy()),
        image_ph_mask=get_photometric_mask(image, verbose=False),
        degree=1.5,
        verbose=False
    )
    plt.figure(figsize=(6, 6))
    ...
    # 5. Apply Color Transfer from Grayscale
    image_colortone = transfer_graytone_to_color(
        image_color=image,
    ...
    plt.figure(figsize=(6, 6))
    plt.imshow(image_colortone, vmin=0, vmax=255)
    plt.title('Color Transfer from Grayscale')
    plt.axis('off')
    plt.tight_layout()
    plt.show()

    # 6. Apply Saturation Adjustment
    image_colortone_saturation = change_color_saturation(
        image_color=image_colortone,
        image_ph_mask=get_photometric_mask(image, verbose=False),
        sat_degree=1.2,
    ...
    # 7. Create Photometric Mask (This is an intermediate step, so it's displayed before other techniques)
    image_ph_mask = get_photometric_mask(image, verbose=False)

    # 8. Plot Color Histogram Analysis (The histograms will be plotted separately for each channel)
    plot_histogram(image, "Histogram of Input Image")
    plot_histogram(image_colortone_saturation, "Histogram of Enhanced Image")
